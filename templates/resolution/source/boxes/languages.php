...
$curr_lng = $lng->catalog_languages[$_SESSION['language_code']];
$curr_lng_icon = xtc_image('lang/'.$curr_lng['directory'] .'/'.$curr_lng['image'], $curr_lng['name']);
...
while (list($key, $value) = each($lng->catalog_languages)) {
  ...
  $languages_string .= ' <a href="' . xtc_href_link(basename($PHP_SELF), xtc_get_all_get_params(array('language', 'currency')) . 'language=' . $key, $request_type) . '">' . xtc_image('lang/' .  $value['directory'] .'/' . $value['image'], $value['name']) . $value['name'] . '</a> ';
}
...
if ($count_lng > 1 ) {
  ...
  $box_content='';
  $box_smarty->assign('curr_lng_icon', $curr_lng_icon);
